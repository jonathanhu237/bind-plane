...
export function ReleaseConsole() {
  ...
  const preparationJobQuery = useQuery({
    ...
    queryFn: () => apiRequest<ReleaseJob>(`/releases/jobs/${preparationJobId}`, token),
    enabled: Boolean(token && preparationJobId && preparationSeed),
    refetchInterval: (query) => {
      const status = query.state.data?.status;
      return status && !terminalStatuses.has(status) && status !== "waiting_confirmation" ? 2000 : false;
    },
  });
  ...
  const prepareMutation = useMutation({
    mutationFn: (values: ReleasePrepareValues) =>
      apiRequest<ReleasePreparation>("/releases/prepare", token, {
        ...
        body: JSON.stringify({
          ...
          force: values.force,
          selected_switch_id:
            values.force && values.selectedSwitchId ? values.selectedSwitchId : undefined,
        }),
      }),
    onSuccess: (response) => {
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "Preparation failed"),
  });
  ...
  const createJobMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "Job creation failed"),
  });
  ...
  const showForcedSwitchSelector =
    isAdmin && forceValue && !hasPreparedJob && (preparation?.status ?? null) !== "ready";
      ...
      <Card>
        ...
        <CardContent>
          <form className="grid gap-4" onSubmit={form.handleSubmit(submit)}>
            <FormField label="IPv4 address" error={form.formState.errors.targetIp}>
              <Input placeholder="10.44.132.254" {...form.register("targetIp")} />
            </FormField>
            <FormField label="Reason" error={form.formState.errors.reason}>
            ...
            <FormField label="Ticket ID" error={form.formState.errors.ticketId}>
            ...
            {showForcedSwitchSelector ? (
              <FormField label="Forced switch">
                <Select {...form.register("selectedSwitchId")}>
            ...
            <div className="grid gap-4">
              ...
              <dl className="grid gap-3 text-sm md:grid-cols-2">
                {[
                  ...
                  ["Reason", reasonLabels[preparation.reason ?? form.getValues("reason")]],
                  ...
                  <div key={term} className="rounded-md border bg-muted/30 p-3">
                    ...
                    <dd className={cn("mt-1 break-words font-medium", term === "Force" && displayedForce && "text-destructive")}>
                      {value}
                    </dd>
                  </div>
                ))}
              ...
              {preparation.observation ? (
                <details>
                  <summary className="cursor-pointer text-sm font-medium">Raw pre-query output</summary>
                  <pre className="mt-2">{preparation.observation.raw_output}</pre>
                </details>
              ) : null}
              <div className="flex flex-wrap gap-2">
                ...
                {canForceStoppedNoRecord ? (
                  <Button
                    ...
                    type="button"
                    variant="destructive"
                    onClick={() => createJobMutation.mutate({ forceOverride: true })}
                  >
                    <Play size={16} />
                    Force release job
                  </Button>
